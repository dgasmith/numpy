...
def einsum(*operands, **kwargs):
    ...
    *operands : list of array_like
    ...
    if '.' in subscripts:
        ellipse_inds = None
        split_subscripts = subscripts.split('->')[0].split(',')
        for num, sub in enumerate(split_subscripts):
            if '.' in sub:
                if ((sub.count('.') % 3) != 0) or (sub.count('...') != 1):
                    raise ValueError("Invalid Ellipses.")

                ellipse_count = len(operands[num].shape) - (len(sub) - sub.count('.'))
                if ellipse_inds is None:
                    unused = list(einsum_symbols_set - set(sub))
                    ellipse_inds = ''.join(unused[:ellipse_count])

                # This isnt quite right, ellipses are a bit over generalized.
                # Need to think on this a bit more
                elif len(ellipse_inds) != ellipse_count:
                    raise ValueError("Ellipses must be of the same size")
        if ellipse_inds is None:
            raise ValueError("Invalid Ellipses.")
        else:
            subscripts = subscripts.replace('...', ellipse_inds)
